...
def start(response,question,prompt1):
    ...
    if response["information_number"]=="1" or response["information_number"]==1:
        ...
        for kkk in list(key):
            ...
            data={
                "query_conds":  {"统一社会信用代码": f"{kkk}"},
                "need_fields": ["公司全称"]  # 假设你想要获取公司全称
            }
            result = api(data, "get_company_register_name")
            if result != []:
                answ.append(result)
        ...
        for kkk in list(key):
            ...
            if t!=[]:
                answ.append(t)
    return answ
question="深圳市佳士科技股份有限公司的法人与天津凯发电气股份有限公司的法人发生了产品生产者责任纠纷，深圳市佳士科技股份有限公司委托给了山东崇义律师事务所，天津凯发电气股份有限公司委托给了山东海金州律师事务所，请写一份民事起诉状给辽宁省沈阳市中级人民法院时间是2024-04-02，注：法人的地址电话可用公司的代替。"
if "整合报告" in question:
    report(question)
elif "民事起诉状" in question:
    result = generate_lawsuit_info(question)
    print("生成的民事起诉状信息:", result)
else:
