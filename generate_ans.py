...
def generate_ans(content,response_message,rsp):
    ...
    print(ans)
    response_type = ans.tool_calls    # 解析response中返回的JSON字符串
    ...
    while response_type !=None:
        ...
        messages.append({
            "role": "user",
            "content": content+"。关于这个问题，你可以继续调用相应工具获取更多信息，你认为信息足够做出推理时请给出回答，或许需要利用数学知识以及逻辑推理在回答过程中"
        })
        messages.append({
            "role": "tool",
            "content": f"{answer}",
        ...
        messages.append( {
                "role": "user",
                "content":"如果你确认得到了最终结果，请直接回答我；如果你需要继续调用工具，请直接发出相应的tool_calls请求消息"
            })      
        ans = (client.chat.completions.create(
            model="glm-4", # 填写需要调用的模型名称
            messages=messages,
        ...
        print(ans)
        response_type = ans.tool_calls 
        while re.search(r'tool_calls', ans.content, re.IGNORECASE):
            messages.append( {
                "role": "user",
                "content":"如果你需要调用工具，请直接发出相应的tool_calls请求消息"
            })      
